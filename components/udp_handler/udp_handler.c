...
static void udp_client_task(void *pvParameters) {
  ...
  while (1) {
    ...
    ESP_LOGI(TAG, "Attempting to send to %s:%d", inet_ntoa(dest_addr.sin_addr),
             ntohs(dest_addr.sin_port));

    char message[] = "Hello Stefan\n"; // Add explicit newline
    ...
    ESP_LOGI(TAG, "Message sent to %s:%d", HOST_IP_ADDR, PORT);
